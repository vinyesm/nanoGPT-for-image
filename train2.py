...
def train(
    ...
    log_media = False, # if True dumps generated image examples in examples folder (and wandb when activated)
    # data
    ...
    train function
    '''
    config = {k:v for k , v in locals().items() if not k.startswith('_') and isinstance(v, (int, float, bool, str))} # will be useful for logging
...
if len(sys.argv) > 1:
    config_path = sys.argv[1].replace('/', '.').replace('.py', '')
    config = importlib.import_module(config_path)
    train_args = {k: getattr(config, k) for k in train.__code__.co_varnames if hasattr(config, k)}
    train(**train_args)
else:
    train()
